                       ...
                       <table class="table table-sm table-striped">
                        ...
                        <tbody>
                          <tr v-for="card in cards">
                            <th>{{card.pro_name}}</th>
                            <td>
                              <button class="btn btn-sm btn-success" @click.prevent="Increment(card.id)">+</button>
                              <input type="text" readonly="" :value="card.pro_quantity" style="width:20px;" name="">
                              <button class="btn btn-sm btn-danger" @click.prevent="Decrement(card.id)" v-if="card.pro_quantity>=2">-</button>
                              <button class="btn btn-sm btn-danger" v-else="" disabled="">-</button>
                            </td>
                            <td>{{card.product_price}}</td>
                            <td>{{card.sub_total}}</td>
                            <td><a @click="removeToCart(card.id)" class="btn btn-sm btn-danger">x</a></td>
                          </tr>
                        </tbody>
                      </table>
                      <hr>
                    ...
                    <div class="card-footer">
                      <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                         ...
                         <li class="list-group-item d-flex justify-content-between align-items-center">
                          Total:
                         <strong> {{subTotal+(subTotal*vats.vat*0.01)}} Tk</strong>
                        </li>
                      </ul>
                      <br>
              ...
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Customer Insert</h5>
      ...
      mounted(){
        ...
        computed:{
         ...
         subTotal(){
          let sub_total=0;
          for(let i=0;i<this.cards.length;i++){
            sub_total+=parseFloat(this.cards[i].pro_quantity)*parseFloat(this.cards[i].product_price);
        ...
        // Cart Method
          ...
          removeToCart(id){
            axios.get('/api/remove-to-cart/'+id)
            .then(()=>{
              Reload.$emit('AfterAdd');
          ...
          Increment(id){
            axios.get('/api/increment-to-cart/'+id)
            .then(()=>{
              Reload.$emit('AfterAdd');
              Notification.success()
            })
          ...
          Decrement(id){
            axios.get('/api/decrement-to-cart/'+id)
            .then(()=>{
              Reload.$emit('AfterAdd');
              Notification.success()
            })
        ...
        onFileselected(event){
          ...
          orderDone(){
            let total=this.subTotal+(this.subTotal*this.vats.vat*0.01);
